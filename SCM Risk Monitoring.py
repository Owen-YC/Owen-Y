...
# DeepL Translator 준비 (API 키는 보안을 위해 환경변수나 st.secrets 사용을 권장합니다)
try:
    # streamlit 클라우드에 배포 시 st.secrets 사용
    # auth_key = st.secrets["deepl"]["auth_key"]
    auth_key = "844bd509-6c74-4a16-bfb1-1498e2729fb7:fx" # 예: "844bd509-6c74-4a16-bfb1-1498e2729fb7:fx"
    translator = deepl.Translator(auth_key)
    # API 키 유효성 검사
    translator.get_usage()
    st.session_state.deepl_available = True
except Exception as e:
    st.session_state.deepl_available = False
    translator = None
    # st.warning(f"DeepL API 초기화 실패: {e}. 번역 기능이 비활성화됩니다.") # 필요시 경고 메시지

# Gemini 클라이언트 초기 설정 (앱 로드 시 1회만 실행)
if 'gemini_client_initialized' not in st.session_state:
    try:
        # 제공된 API 키를 하드코딩하여 사용
        api_key = "AIzaSyCJ1F-HMS4NkQ64f1tDRqJV_N9db0MmKpI"
        client = genai.Client(api_key=api_key)
        # API 키 유효성 검사를 위해 간단한 호출 시도 (예: 모델 리스트)
        client.models.list() 
        ...
        # st.warning(f"Gemini API 초기화 실패: {e}. 동적 키워드 생성이 비활성화됩니다.")
    st.session_state.gemini_client_initialized = True
...
def get_dynamic_keywords_from_url(article_url: str) -> Dict:
    """
    단일 URL에 대해 Gemini API를 호출하여 키워드를 추출하고, 결과 JSON 또는 에러 메시지를 반환합니다.
    """
    ...
    try:
        # 제공된 코드의 모델, contents, tools, config 구조를 그대로 사용합니다.
        model = "gemini-2.5-flash-lite"
        contents = [types.Content(role="user", parts=[types.Part.from_text(text=article_url)])]
        ...
        generate_content_config = types.GenerateContentConfig(
            ...
            tools=tools,
            system_instruction=[
                types.Part.from_text(text="""유저가 입력한 기사 URL 안의 내용을 읽고, 그 기사의 핵심 내용을 알 수 있도록 핵심 키워드 5개를 추출할 것.
이때, 다음과 같은 key 값을 가진 Json 형식으로 추출할 것 :
 - \"keyword_1\"
 - \"keyword_2\"
 - \"keyword_3\"
 - \"keyword_4\"
 - \"keyword_5\""""),
        ...
        # 응답에서 JSON 부분만 파싱하여 추출합니다.
        json_start = full_response_text.find('{')
        json_end = full_response_text.rfind('}')
        if json_start != -1 and json_end != -1:
            json_string = full_response_text[json_start:json_end + 1]
            return json.loads(json_string)
        else:
            return {"error": "Valid JSON not found in response", "response": full_response_text}

    except Exception as e:
        # API 호출 또는 처리 중 에러가 발생하면 에러 정보를 담은 dict를 반환합니다.
        return {"error": f"An exception occurred: {e}"}

def generate_keywords_for_articles_in_parallel(articles: List[Dict]) -> List[Dict]:
    """
    ThreadPoolExecutor를 사용하여 여러 기사의 키워드를 병렬로 생성합니다.
    API 호출 실패 시 기존 로직(extract_keywords_from_title)으로 대체합니다.
    """
    # 키워드가 이미 생성된 기사는 제외하고, 생성 필요한 기사만 필터링
    articles_to_process = [article for article in articles if 'keywords_generated' not in article]

    if not articles_to_process or not st.session_state.get('gemini_available', False):
        return articles # 처리할 기사가 없거나 Gemini API가 비활성화면 그대로 반환

    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        # URL별로 작업을 제출하고, future와 원본 article을 매핑
        future_to_article = {executor.submit(get_dynamic_keywords_from_url, article['url']): article for article in articles_to_process}

        for future in concurrent.futures.as_completed(future_to_article):
            article = future_to_article[future]
            try:
                result = future.result()
                # 성공적으로 JSON을 받았는지 확인 (error 키가 없는지)
                if "error" not in result and all(f"keyword_{i}" in result for i in range(1, 6)):
                    # 성공: Gemini가 생성한 키워드 사용
                    keywords = [f"#{result[f'keyword_{i}']}" for i in range(1, 6)]
                    article['keywords'] = keywords
                else:
                    # 실패: 기존 로직으로 대체
                    article['keywords'] = extract_keywords_from_title(article['title'])
            except Exception:
                 # 실패: 기존 로직으로 대체
                article['keywords'] = extract_keywords_from_title(article['title'])
            
            # 처리 완료 플래그 설정 (중복 API 호출 방지)
            article['keywords_generated'] = True
            
    return articles

def translate_text(text: str, target_language: str = 'ko') -> str:
    """DeepL API를 사용하여 텍스트를 번역합니다."""
    # DeepL translator가 비활성화 상태이거나 텍스트가 비어있으면 원본 텍스트 반환
    if not st.session_state.get('deepl_available', False) or not text or not text.strip():
        return text

    try:
        # DeepL API에서 사용하는 언어 코드로 변환
        target_lang_code = "KO" if target_language == 'ko' else "EN-US"
        
        result = translator.translate_text(text, target_lang=target_lang_code)
        return result.text
    except Exception as e:
        # 번역 실패 시 원본 텍스트 반환
        # st.error(f"DeepL 번역 오류: {e}") # 디버깅 시 사용
        return text
...
def translate_korean_to_english(korean_text: str) -> str:
    """DeepL API를 사용하여 한국어 텍스트를 영어로 번역합니다."""
    # DeepL translator가 비활성화 상태이거나 텍스트가 비어있으면 원본 텍스트 반환
    if not st.session_state.get('deepl_available', False) or not korean_text or not korean_text.strip():
        return korean_text
...
def main():
    ...
    with col1:
        ...
        # 페이지네이션 설정
        articles_per_page = 10  # 페이지 당 기사 수를 10개로 제한
        total_articles = len(sorted_articles)
        total_pages = (total_articles + articles_per_page - 1) // articles_per_page
        if total_pages == 0: total_pages = 1 # 기사가 없을 경우 1페이지로 표시

        # 현재 페이지 설정 (기본값: 1)
        if 'current_page' not in st.session_state:
            st.session_state.current_page = 1
        # 페이지가 범위를 벗어나지 않도록 보정 (예: 검색 후 기사 수가 줄었을 때)
        ...
        articles_for_page = sorted_articles[start_index:end_index]

        # Spinner와 함께 현재 페이지 기사의 키워드 동적 생성
        with st.spinner("✨ AI로 최신 뉴스 태그를 생성하는 중..."):
            # 병렬 처리 함수를 호출하여 키워드 생성 (이미 생성된 것은 건너뜀)
            articles_for_page = generate_keywords_for_articles_in_parallel(articles_for_page)

        # 뉴스 리스트 (Motion 효과 + 해시태그)
        for i, article in enumerate(articles_for_page, 1):
            # 키워드 안전하게 처리
            if 'keywords' in article and article['keywords']:
                keywords = article['keywords']
            else:
                # Gemini가 비활성화되었거나 태그가 없는 경우, 기존 로직으로 키워드 생성
